...
int main() {
    ...
    auto jacobi_theta_coarse = [](CoarseReal q) {
        return boost::math::jacobi_theta3<CoarseReal>(0.0, q);
    };
    auto jacobi_theta_precise = [](PreciseReal q) {
        return boost::math::jacobi_theta3<PreciseReal>(0.0, q);
    };

    std::string filename = "jacobi_theta3q_" + boost::core::demangle(typeid(CoarseReal).name()) + ".svg";
    int samples = 2500;
    int width = 1100;
    ...
    std::string title = "jacobi_theta3q ULP plot at " + boost::core::demangle(typeid(CoarseReal).name()) + " precision";
    //plot.title(title);
