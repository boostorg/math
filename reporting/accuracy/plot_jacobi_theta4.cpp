...
int main() {
    ...
    auto jacobi_theta_coarse = [](CoarseReal z) {
        return boost::math::jacobi_theta4m1<CoarseReal>(z, 0.05);
    };
    auto jacobi_theta_precise = [](PreciseReal z) {
        return boost::math::jacobi_theta4m1<PreciseReal>(z, 0.05);
    };

    std::string filename = "jacobi_theta4_" + boost::core::demangle(typeid(CoarseReal).name()) + ".svg";
    int samples = 2500;
    int width = 1100;
    ...
    std::string title = "jacobi_theta4 ULP plot at " + boost::core::demangle(typeid(CoarseReal).name()) + " precision";
    //plot.title(title);
