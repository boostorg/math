...
void test_integer_mean()
{
    ...
    CHECK_MOLLIFIED_CLOSE(mu, 3, tol);
    ...
    CHECK_MOLLIFIED_CLOSE(m1, m2, tol*abs(m1));
}
...
void test_mean()
{
    ...
    CHECK_MOLLIFIED_CLOSE(mu, Real(3), tol);
    ...
    CHECK_MOLLIFIED_CLOSE(mu, Real(2), tol);
    ...
    CHECK_MOLLIFIED_CLOSE(mu, Real(4), tol);
    ...
    for (size_t i = 1; i < 30; ++i)
    {
        ...
        CHECK_MOLLIFIED_CLOSE(higham_, naive_, 100*tol*abs(naive_));
    }
}
...
{
    Real tol = std::numeric_limits<Real>::epsilon();
    std::vector<Reactor<Real>> v(5);

    for (size_t i = 0; i < v.size(); ++i) {
        v[i] = {Real(i), 2*Real(i)};
    }
    Real temp_mu = boost::math::tools::mean(v, &Reactor<Real>::temperature);
    CHECK_MOLLIFIED_CLOSE(temp_mu, 2, tol);

    Real pressure_mu = boost::math::tools::mean(v, &Reactor<Real>::pressure);
    CHECK_MOLLIFIED_CLOSE(pressure_mu, 4, tol);
}
...
void test_complex_mean()
{
    ...
    CHECK_MOLLIFIED_CLOSE(mu.imag(), Real(3), tol);
    CHECK_MOLLIFIED_CLOSE(mu.real(), Real(0), tol);
...
void test_variance()
{
    ...
    CHECK_MOLLIFIED_CLOSE(sigma_sq, 1/Real(4), tol);
    ...
    for (size_t j = 16; j < 2048; j *= 2)
    {
        ...
        CHECK_MOLLIFIED_CLOSE(sigma_sq, (n*n-1)/Real(12), tol*sigma_sq);
    }
...
void test_integer_variance()
{
    ...
    CHECK_MOLLIFIED_CLOSE(sigma_sq, double(0), tol);

    std::forward_list<Z> l{0,1,0,1,0,1,0,1};
    sigma_sq = boost::math::tools::variance(l.begin(), l.end());
    CHECK_MOLLIFIED_CLOSE(sigma_sq, 1.0/4.0, tol);
...
void test_integer_skewness()
{
    ...
    CHECK_MOLLIFIED_CLOSE(skew, 0.0, tol);
    ...
    skew = boost::math::tools::skewness(v);
    CHECK_MOLLIFIED_CLOSE(skew, 3.0/2.0, tol);
...
void test_skewness()
{
    ...
    CHECK_MOLLIFIED_CLOSE(skew, Real(0), tol);
    ...
    // mu = 1, sigma^2 = 4, sigma = 2, skew = 3/2
    skew = boost::math::tools::skewness(v);
    CHECK_MOLLIFIED_CLOSE(skew, Real(3)/Real(2), tol);
...
void test_kurtosis()
{
    ...
    CHECK_MOLLIFIED_CLOSE(kurt, Real(0), tol);

    v = {1,2,3,4,5};
    // mu =1, sigma^2 = 2, kurtosis = 17/10
    kurt = boost::math::tools::kurtosis(v);
    CHECK_MOLLIFIED_CLOSE(kurt, Real(17)/Real(10), tol);

    v = {0,0,0,0,5};
    // mu = 1, sigma^2 = 4, sigma = 2, skew = 3/2, kurtosis = 13/4
    kurt = boost::math::tools::kurtosis(v);
    CHECK_MOLLIFIED_CLOSE(kurt, Real(13)/Real(4), tol);
    ...
    CHECK_MOLLIFIED_CLOSE(kurt, Real(3), 0.1);
    ...
    CHECK_MOLLIFIED_CLOSE(excess_kurtosis, -6.0/5.0, 0.2);
    ...
    //CHECK_MOLLIFIED_CLOSE(excess_kurtosis,  6.0, 0.2);
}
...
void test_integer_kurtosis()
{
    ...
    CHECK_MOLLIFIED_CLOSE(kurt, double(0), tol);

    v = {1,2,3,4,5};
    // mu =1, sigma^2 = 2, kurtosis = 17/10
    kurt = boost::math::tools::kurtosis(v);
    CHECK_MOLLIFIED_CLOSE(kurt, 17.0/10.0, tol);

    v = {0,0,0,0,5};
    // mu = 1, sigma^2 = 4, sigma = 2, skew = 3/2, kurtosis = 13/4
    kurt = boost::math::tools::kurtosis(v);
    CHECK_MOLLIFIED_CLOSE(kurt, 13.0/4.0, tol);
...
void test_first_four_moments()
{
    ...
    CHECK_MOLLIFIED_CLOSE(M1_1, Real(1), tol);
    CHECK_MOLLIFIED_CLOSE(M2_1, Real(0), tol);
    CHECK_MOLLIFIED_CLOSE(M3_1, Real(0), tol);
    CHECK_MOLLIFIED_CLOSE(M4_1, Real(0), tol);

    v = {1, 2, 3, 4, 5};
    auto [M1_2, M2_2, M3_2, M4_2] = boost::math::tools::first_four_moments(v);
    CHECK_MOLLIFIED_CLOSE(M1_2, Real(3), tol);
    CHECK_MOLLIFIED_CLOSE(M2_2, Real(2), tol);
    CHECK_MOLLIFIED_CLOSE(M3_2, Real(0), tol);
    CHECK_MOLLIFIED_CLOSE(M4_2, Real(34)/Real(5), tol);
}
...
void test_sample_gini_coefficient()
{
    ...
    CHECK_MOLLIFIED_CLOSE(gini, Real(1), tol);
    ...
    CHECK_MOLLIFIED_CLOSE(gini, Real(0), tol);
...
void test_gini_coefficient()
{
    ...
    CHECK_MOLLIFIED_CLOSE(gini, expected, tol);
    ...
    gini = boost::math::tools::gini_coefficient(v);
    CHECK_MOLLIFIED_CLOSE(gini, Real(expected), 0.01);
...
void test_integer_gini_coefficient()
{
    ...
    v[2] = 1;
    gini = boost::math::tools::gini_coefficient(v.begin(), v.end());
    CHECK_MOLLIFIED_CLOSE(gini, 0.0, tol);
