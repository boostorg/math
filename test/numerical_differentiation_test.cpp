...
void test_order(size_t points_to_test)
{
    ...
    bool failure_rate_acceptable = (double) failures / (double) points_to_test < 0.03;
    BOOST_TEST(failure_rate_acceptable);
}
...
void test_complex_step()
{
    ...
    while ( x < 200 )
    {
        ...
        {
            bool small = computed < std::numeric_limits<Real>::epsilon();
            BOOST_TEST(small);
        }
        else
        {
    ...
    //std::cout << "Derivative of f1 at x = " << x << " (finite diffe): " << finite_difference_derivative<decltype(moler_example<Real>), Real, 2>(moler_example<Real>, x, &error) << std::endl;
    //std::cout << "Error estimate " << error << std::endl;
    //std::cout << "Derivative of f1 at x = " << x << " (finite diffe): " << finite_difference_derivative<decltype(moler_example<Real>), Real, 4>(moler_example<Real>, x, &error) << std::endl;
    //std::cout << "Error estimate " << error << std::endl;
    //std::cout << "Derivative of f1 at x = " << x << " (finite diffe): " << finite_difference_derivative<decltype(moler_example<Real>), Real, 6>(moler_example<Real>, x, &error) << std::endl;
    //std::cout << "Error estimate " << error << std::endl;
    //std::cout << "Derivative of f1 at x = " << x << " (finite diffe): " << finite_difference_derivative<decltype(moler_example<Real>), Real, 8>(moler_example<Real>, x, &error) << std::endl;
    //std::cout << "Error estimate " << error << std::endl;
}
...
BOOST_AUTO_TEST_CASE(numerical_differentiation_test)
{
    ...
    #ifdef __GNUC__
    #ifndef __clang__
    test_complex_step<float128>();
    #endif
    #endif
    test_complex_step<cpp_bin_float_100>();
    test_complex_step<cpp_dec_float_100>();
    ...
    #ifdef __GNUC__
    #ifndef __clang__
    test_bessel<float128>();
    #endif
    #endif

    size_t points_to_test = 1000000;
