 ...
 *
 * Module build check for boost::math::statistics::anderson_darling_normality_statistic.
 * It exercises the exported normality-statistic entry point when the test is built
 * against the boost.math module. The double expected value and the scale/shift
 * invariance checks mirror the assertions in anderson_darling_test.cpp.
 */
...
// The Anderson-Darling normality statistic agrees with Mathematica's
// AndersonDarlingTest and is invariant under affine rescaling of the data.
void test_ad_normal_agreement_w_mathematica()
{
    ...
    // Scaling the data and the supplied standard deviation leaves the statistic unchanged.
    double sigma = 2;
    for (auto & x : v)
    {
        x *= sigma;
    ...
    // Shifting the data and the supplied mean leaves the statistic unchanged.
    double mu = 10;
    for (auto & x : v)
    {
        x += mu;
