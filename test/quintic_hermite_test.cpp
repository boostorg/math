...
void test_cardinal_derivative_scaling()
{
    ...
    {
        std::vector<std::array<double, 3>> data(4);
        std::vector<double> y(4), dy(4), d2y(4);
        for (size_t i = 0; i < data.size(); ++i)
        {
            double x = -1 + i*dx;
            data[i] = {x*x*x, 3*x*x, 6*x};
            y[i] = x*x*x;
            dy[i] = 3*x*x;
            d2y[i] = 6*x;
        }
        auto f = boost::math::interpolators::detail::cardinal_quintic_hermite_detail_aos(std::move(data), -1.0, dx);
        auto g = boost::math::interpolators::detail::cardinal_quintic_hermite_detail(std::move(y), std::move(dy), std::move(d2y), -1.0, dx);
        for (int i = 0; i <= 24; ++i)
        {
            double x = -1 + i*dx/8;
