...
{
    template <typename T>
    static BOOST_GPU_ENABLED T result(T base)
    {
...
{
    template <typename T>
    static BOOST_GPU_ENABLED T result(T base){ return base; }
};
...
struct power_if_positive
{
    ...
    static BOOST_GPU_ENABLED T result(T base, const Policy&)
...
{
    template <typename T, class Policy>
    static BOOST_GPU_ENABLED T result(T base, const Policy& policy)
    {
        if (base == 0)
        {
...
inline BOOST_GPU_ENABLED typename tools::promote_args<T>::type pow(T base, const Policy& policy)
{ 
...
inline BOOST_GPU_ENABLED typename tools::promote_args<T>::type pow(T base)
