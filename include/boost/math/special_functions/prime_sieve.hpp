...
inline decltype(auto) prime_sieve(ExecutionPolicy&& exec, Integer upper_bound, T output)
{
...
template<typename ExecutionPolicy, typename Integer, typename T>
inline decltype(auto) prime_range(ExecutionPolicy&& exec, Integer upper_bound, Integer lower_bound, T output)
{
    if constexpr (detail::prime_sieve::is_container_t<std::remove_pointer_t<T>>)
    {
        if constexpr (std::is_integral_v<Integer>)
