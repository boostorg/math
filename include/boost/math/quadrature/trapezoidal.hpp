...
auto trapezoidal(F f, Real a, Real b, const decltype(f(a))& zero, Norm norm, Real tol, std::size_t max_refinements, Real* error_estimate, Real* L1, const Policy& pol)->decltype(static_cast<Real>(norm(f(a))), f(a))
{
    static const char* function = "boost::math::quadrature::trapezoidal<%1%>(F, %1%, %1%, %1%)";
    ...
    if (a == b)
    {
        if (error_estimate) *error_estimate = Real(0);
        if (L1) *L1 = Real(0);
        return zero;
    }
    if(a > b)
    {
        return -trapezoidal(f, b, a, zero, norm, tol, max_refinements, error_estimate, L1, pol);
    }
    ...
    Real h = (b - a)*half<Real>();
    K I0 = (ya + yb)*h;
    Real IL0 = (magnitude(ya) + magnitude(yb))*h;

    K yh = f(a + h);
    K I1 = zero;
    I1 = I0*half<Real>() + yh*h;
    Real IL1 = IL0*half<Real>() + magnitude(yh)*h;
