...
public:
    ...
    void backward()
    {
        ...
        for (size_t i = 0; i < n_; ++i) {
            update_argument_adj_v(i, get_adjoint_v() + get_derivative_v(i) * get_adjoint_v());
        }
    }
};
template<typename T, class derived_expression>
    ...
    : public expression<T, abstract_binary_expression<T, LHS, RHS, concrete_binary_operation>>
{
    ...
    void propagatex(gradient_node<T>* node, T adj) const
    {
        ...
        //if constexpr (num_lhs_args) {
        //    lhs.template propagatex<arg_index>(node, adj * partial_l);
        //}
        //if constexpr (num_rhs_args) {
        //    rhs.template propagatex<arg_index + num_lhs_args>(node, adj * partial_r);
        //}
...
namespace detail {
/* helper functions for creating nested rvars for
...
template<typename T>
struct inner_type<rvar<T>>
{
    using type = typename inner_type<T>::type;
};
template<typename T>
typename inner_type<T>::type get_inner_value(T const& arg)
{
    return arg;
}

template<typename T>
typename inner_type<T>::type get_inner_value(rvar<T> const& arg)
{
    return get_inner_value(arg.item());
}
template<typename T>
typename inner_type<T>::type get_inner_adj(T const& arg)
{
    return arg;
}

template<typename T>
typename inner_type<T>::type get_inner_adj(rvar<T> const& arg)
{
    return get_inner_value(arg.adjoint());
}
// Base case: depth == 0
template<typename T>
auto make_rvar_impl(T value, std::integral_constant<size_t, 0>)
{
    return rvar<T>(value);
}
...
template<typename T,
         size_t depth,
         typename = std::enable_if_t<!std::is_base_of<rvar<typename inner_type<T>::type>, T>::value>>
auto make_rvar_impl(T value, std::integral_constant<size_t, depth>)
{
    auto inner_rvar = make_rvar_impl(value, std::integral_constant<size_t, depth - 1>{});
    return rvar<decltype(inner_rvar)>(inner_rvar);
}

template<typename T, size_t depth>
struct make_rvar_type
{
    using type = rvar<typename make_rvar_type<T, depth - 1>::type>;
};

template<typename T>
struct make_rvar_type<T, 0>
{
    using type = rvar<T>;
};
} // namespace detail
...
template<typename T, size_t depth>
auto make_rvar_2(T value)
{
    return detail::make_rvar_impl(value, std::integral_constant<size_t, depth>{});
}
...
private:
    ...
    void make_leaf_node()
    {
        ...
        //auto& t2 = get_tape_manager<T, MAX_DEPTH>().template get<depth_>();

        //std::cout << depth_ << std::endl;
        //auto& tape = get_tape_manager<T, MAX_DEPTH>().template get<depth_>()
        //node_      = tape.emplace_leaf_node();
    }
    ...
    template<typename E>
    void make_rvar(const expression<T, E>& expr)
    {
...
public:
    ...
    template<class E>
    rvar(const expression<T, E>& expr)
    {
        value_ = expr.evaluate();
        make_rvar(expr);
    }

    /* helper functions for returning nested types */
    /* T is the inner type, i.e. rvar<rvar<U>> -> T is rvar<U> */
    T evaluate() const { return value_; };
    T item() const { return value_; };

    /* auto for innermost type, i.e. rvar<rvar<U>> -> U */
    /* same type as typename detail::get_inner_value::type */
    auto item_v() const { return detail::get_inner_value(this->item()); };
    T    adjoint() const { return *node_->adjoint_; };
    auto adjoint_v() const { return detail::get_inner_adj(this->adjoint()); };
