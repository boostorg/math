...
{
    constexpr auto sixth = boost::math::constants::half<Real>()*boost::math::constants::third<Real>();
    auto absx = std::abs(x);
    if (absx < 1)
    {
        auto y = 2 - absx;
...
template<class Real>
Real b3_spline_derivative(Real x)
{
    if ( x < 0 )
    {
        return -b3_spline_derivative(-x);
    }
                                     ...
                                     Real left_endpoint_derivative, Real right_endpoint_derivative) : m_a(left_endpoint), m_avg(0)
{
    ...
    {
        a1 = m_h_inv*(-25*f[0]/12 + 4*f[1] - 3*f[2] + 4*f[3]/3 - f[4]/4);
    }
    ...
    {
        auto n = length - 1;
        b1 = -m_h_inv*(25*f[n - 4]/12 - 4*f[n - 3] + 3*f[n - 2] - 4*f[n - 1]/3 + f[n]/4);
    }
...
Real cubic_b_spline<Real>::interpolate_at(Real x) const
{
    ...
    size_t k_min = (size_t) std::max(static_cast<long>(0), static_cast<long>(std::ceil(t - 2)));
    size_t k_max = (size_t) std::min(static_cast<long>(m_beta.size() - 1), static_cast<long>(std::floor(t + 2)));
...
Real cubic_b_spline<Real>::interpolate_derivative(Real x) const
{
    ...
    for (size_t k = k_min; k <= k_max; ++k)
    {
        z += m_beta[k]*b3_spline_derivative(t - k);
    }
    return z;
}
