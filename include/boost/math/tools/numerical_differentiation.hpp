...
Real complex_step_derivative(const F f, Real x)
{
    ...
    // Note that some authors recommend taking the stepsize h to be smaller than epsilon().
    // This was tested over a few billion test cases and found the make the error worse.
    // Even 2eps and eps/2 made the error worse, which was surprising.
    using std::complex;
    complex<Real> fp = f(complex<Real>(x, std::numeric_limits<Real>::epsilon()))/ std::numeric_limits<Real>::epsilon();
    return fp.imag();
}
