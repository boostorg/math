...
template <class T>
BOOST_MATH_HOST_DEVICE T chebyshev_coefficient(unsigned n, unsigned m)
{
...
template <class Seq>
BOOST_MATH_HOST_DEVICE Seq polynomial_to_chebyshev(const Seq& s)
{
...
BOOST_MATH_HOST_DEVICE T evaluate_chebyshev(const Seq& a, const T& x)
{
   // Clenshaw's formula:
   typedef typename Seq::difference_type difference_type;
   T yk2 = 0;
...
BOOST_MATH_HOST_DEVICE typename std::enable_if<!std::numeric_limits<T>::is_integer, void >::type
...
BOOST_MATH_HOST_DEVICE T integer_power(T t, N n)
{
   switch(n)
   {
   case 0:
 ...
 */
template <typename T>
BOOST_MATH_HOST_DEVICE std::pair< polynomial<T>, polynomial<T> >
...
{
   template <class T>
   BOOST_MATH_HOST_DEVICE T operator()(T const &x) const
   {
      return -x;
   }
};
...
struct plus
{
   ...
   BOOST_MATH_HOST_DEVICE T operator()(T const &x, U const& y) const
   {
      return x + y;
   }
};
 ...
 */
template <class T>
BOOST_MATH_HOST_DEVICE polynomial<T> zero_element(std::multiplies< polynomial<T> >)
{
    return polynomial<T>();
}

template <class T>
BOOST_MATH_HOST_DEVICE polynomial<T> identity_element(std::multiplies< polynomial<T> >)
{
...
public:
   ...
   // construct:
   BOOST_MATH_HOST_DEVICE polynomial()= default;

   template <class U>
   BOOST_MATH_HOST_DEVICE polynomial(const U* data, unsigned order)
   ...
   BOOST_MATH_HOST_DEVICE polynomial(Iterator first, Iterator last)
      : m_data(first, last)
   {
       normalize();
   }
   ...
   template <class Iterator>
   BOOST_MATH_HOST_DEVICE polynomial(Iterator first, unsigned length)
      : m_data(first, std::next(first, length + 1))
   {
       normalize();
   }

   BOOST_MATH_HOST_DEVICE polynomial(std::vector<T>&& p) : m_data(std::move(p))
   {
      normalize();
   }

   template <class U, typename std::enable_if<std::is_convertible<U, T>::value, bool>::type = true>
   BOOST_MATH_HOST_DEVICE explicit polynomial(const U& point)
   {
       if (point != U(0))
   ...
   // move:
   BOOST_MATH_HOST_DEVICE polynomial(polynomial&& p) noexcept
      : m_data(std::move(p.m_data)) { }

   // copy:
   BOOST_MATH_HOST_DEVICE polynomial(const polynomial& p)
      : m_data(p.m_data) { }

   template <class U>
   BOOST_MATH_HOST_DEVICE polynomial(const polynomial<U>& p)
   {
    ...
    BOOST_MATH_HOST_DEVICE explicit polynomial(const Range& r)
       : polynomial(r.begin(), r.end()) 
    {
    }
#endif
    BOOST_MATH_HOST_DEVICE polynomial(std::initializer_list<T> l) : polynomial(std::begin(l), std::end(l))
    {
    }

    polynomial&
    BOOST_MATH_HOST_DEVICE operator=(std::initializer_list<T> l)
    {
   ...
   // access:
   BOOST_MATH_HOST_DEVICE size_type size() const { return m_data.size(); }
   BOOST_MATH_HOST_DEVICE size_type degree() const
   {
       if (size() == 0)
          BOOST_MATH_THROW_EXCEPTION(std::logic_error("degree() is undefined for the zero polynomial."));
       return m_data.size() - 1;
   }
   BOOST_MATH_HOST_DEVICE value_type& operator[](size_type i)
   {
      return m_data[i];
   }
   BOOST_MATH_HOST_DEVICE const value_type& operator[](size_type i) const
   {
      return m_data[i];
   }

   BOOST_MATH_HOST_DEVICE T evaluate(T z) const
   {
      return this->operator()(z);
   }

   BOOST_MATH_HOST_DEVICE T operator()(T z) const
   {
      return m_data.size() > 0 ? boost::math::tools::evaluate_polynomial((m_data).data(), z, m_data.size()) : T(0);
   }
   BOOST_MATH_HOST_DEVICE std::vector<T> chebyshev() const
   {
      return polynomial_to_chebyshev(m_data);
   }

   BOOST_MATH_HOST_DEVICE std::vector<T> const& data() const
   {
       return m_data;
   }

   BOOST_MATH_HOST_DEVICE std::vector<T> & data()
   {
       return m_data;
   }

   BOOST_MATH_HOST_DEVICE polynomial<T> prime() const
   {
#ifdef _MSC_VER
...
#pragma warning(pop)
   ...
   BOOST_MATH_HOST_DEVICE polynomial<T> integrate() const
   {
   ...
   // operators:
   BOOST_MATH_HOST_DEVICE polynomial& operator =(polynomial&& p) noexcept
   {
       m_data = std::move(p.m_data);
       return *this;
   }

   BOOST_MATH_HOST_DEVICE polynomial& operator =(const polynomial& p)
   {
       m_data = p.m_data;
       return *this;
   }
   ...
   template <class U>
   BOOST_MATH_HOST_DEVICE typename std::enable_if<std::is_constructible<T, U>::value, polynomial&>::type operator +=(const U& value)
   {
       addition(value);
       normalize();
       return *this;
   ...
   template <class U>
   BOOST_MATH_HOST_DEVICE polynomial& operator +=(const polynomial<U>& value)
   {
      addition(value);
      normalize();
      return *this;
   ...
   BOOST_MATH_HOST_DEVICE void multiply(const polynomial<U>& a, const polynomial<V>& b) {
       if (!a || !b)
       {
           this->set_zero();
           return;
   ...
   template <typename U>
   BOOST_MATH_HOST_DEVICE polynomial& operator >>=(U const &n)
   {
   ...
   BOOST_MATH_HOST_DEVICE bool is_zero() const
   {
       return m_data.empty();
   }

   // Conversion to bool.
   BOOST_MATH_HOST_DEVICE inline explicit operator bool() const
   {
       return !m_data.empty();
   }

   // Fast way to set a polynomial to zero.
   BOOST_MATH_HOST_DEVICE void set_zero()
   {
       m_data.clear();
   }
   ...
   BOOST_MATH_HOST_DEVICE void normalize()
   {
...
private:
    ...
    BOOST_MATH_HOST_DEVICE polynomial& addition(const U& value, R op)
    {
        if(m_data.size() == 0)
            m_data.resize(1, 0);
    ...
    template <class U>
    BOOST_MATH_HOST_DEVICE polynomial& addition(const U& value)
    {
        return addition(value, detail::plus());
    }

    template <class U>
    BOOST_MATH_HOST_DEVICE polynomial& subtraction(const U& value)
    {
        return addition(value, detail::minus());
    }

    template <class U, class R>
    BOOST_MATH_HOST_DEVICE polynomial& addition(const polynomial<U>& value, R op)
    {
    ...
    template <class U>
    BOOST_MATH_HOST_DEVICE polynomial& addition(const polynomial<U>& value)
    {
        return addition(value, detail::plus());
    }

    template <class U>
    BOOST_MATH_HOST_DEVICE polynomial& subtraction(const polynomial<U>& value)
    {
        return addition(value, detail::minus());
    }

    template <class U>
    BOOST_MATH_HOST_DEVICE polynomial& multiplication(const U& value)
    {
    ...
    template <class U>
    BOOST_MATH_HOST_DEVICE polynomial& division(const U& value)
    {
       std::transform(m_data.begin(), m_data.end(), m_data.begin(), [&](const T& x)->T { return x / value; });
       return *this;
    }
...
template <class T>
BOOST_MATH_HOST_DEVICE inline polynomial<T> operator + (const polynomial<T>& a, const polynomial<T>& b)
{
...
template <class T>
BOOST_MATH_HOST_DEVICE inline polynomial<T> operator + (polynomial<T>&& a, const polynomial<T>& b)
{
   a += b;
   return std::move(a);
}
template <class T>
BOOST_MATH_HOST_DEVICE inline polynomial<T> operator + (const polynomial<T>& a, polynomial<T>&& b)
{
   b += a;
   return b;
}
template <class T>
BOOST_MATH_HOST_DEVICE inline polynomial<T> operator + (polynomial<T>&& a, polynomial<T>&& b)
{
   a += b;
   return a;
}
...
template <class T, class U>
BOOST_MATH_HOST_DEVICE inline typename std::enable_if<std::is_constructible<T, U>::value, polynomial<T> >::type operator + (polynomial<T> a, const U& b)
{
   a += b;
   return a;
}
...
template <class T, class U>
BOOST_MATH_HOST_DEVICE inline typename std::enable_if<std::is_constructible<T, U>::value, polynomial<T> >::type operator % (const polynomial<T>&, const U&)
{
...
BOOST_MATH_HOST_DEVICE inline typename std::enable_if<std::is_constructible<T, U>::value, polynomial<T> >::type operator + (const U& a, polynomial<T> b)
{
   b += a;
   return b;
}
...
template <class T>
BOOST_MATH_HOST_DEVICE bool operator == (const polynomial<T> &a, const polynomial<T> &b)
{
...
BOOST_MATH_HOST_DEVICE polynomial<T> operator >> (polynomial<T> a, const U& b)
{
    a >>= b;
    return a;
}
...
// Unary minus (negate).
template <class T>
BOOST_MATH_HOST_DEVICE polynomial<T> operator - (polynomial<T> a)
{
...
template <class T>
BOOST_MATH_HOST_DEVICE bool odd(polynomial<T> const &a)
{
    return a.size() > 0 && a[0] != static_cast<T>(0);
}

template <class T>
BOOST_MATH_HOST_DEVICE bool even(polynomial<T> const &a)
{
    return !odd(a);
}

template <class T>
BOOST_MATH_HOST_DEVICE polynomial<T> pow(polynomial<T> base, int exp)
{
    if (exp < 0)
...
BOOST_MATH_HOST_DEVICE inline std::basic_ostream<charT, traits>& operator << (std::basic_ostream<charT, traits>& os, const polynomial<T>& poly)
{
   os << "{ ";
