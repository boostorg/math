...
{
   typedef std::integral_constant<int, int(N)> tag_type;
...
{
   return detail::evaluate_rational_c_imp(a, b, z, static_cast<const std::integral_constant<int, int(N)>*>(0));
}

template <std::size_t N, class T, class U, class V>
inline V evaluate_rational(const std::array<T,N>& a, const std::array<U,N>& b, const V& z) BOOST_MATH_NOEXCEPT(V)
{
   return detail::evaluate_rational_c_imp(a.data(), b.data(), z, static_cast<std::integral_constant<int, int(N)>*>(0));
}
