...
inline T constant_feigenbaum<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC((boost::integral_constant<int, N>)))
{
   ...
   // We can't static_assert since static assert is C++11;
   // we'll need to fix this after C++03 deprecation:
   //static_assert(N < 3*1018, "\nThe Feigenbaum constant cannot be computed at runtime; it is too expensive. It is known to 1018 decimal digits; you must request less than that.");
   if (N >= 3*1018) {
      throw std::length_error("\nThe Feigenbaum constant cannot be computed at runtime; it is too expensive. It is known to 1018 decimal digits; you must request less than that.");
   }
   return boost::lexical_cast<T>("4.");
}
