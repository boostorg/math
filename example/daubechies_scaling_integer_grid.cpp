...
std::list<std::vector<Real>> integer_grid()
{
    ...
    for (size_t j = 0; j < eigs.size(); ++j) {
        ...
        assert(abs(sum - pow(-1, j)*boost::math::factorial<Real>(j)) < sqrt(std::numeric_limits<Real>::epsilon()));
...
int main()
{
    ...
    fs << "/*\n"
       ...
       << "    static_assert(p < " << p_max << ", \"Integer grids only implemented up to " << p_max - 1 << ".\");\n"; 


    fs << std::hexfloat;
    write_grid<boost::multiprecision::float128, 2>(fs);
    write_grid<boost::multiprecision::float128, 3>(fs);
    write_grid<boost::multiprecision::float128, 4>(fs);
    write_grid<boost::multiprecision::float128, 5>(fs);
    write_grid<boost::multiprecision::float128, 6>(fs);
    write_grid<boost::multiprecision::float128, 7>(fs);
    write_grid<boost::multiprecision::float128, 8>(fs);
    write_grid<boost::multiprecision::float128, 9>(fs);
    write_grid<boost::multiprecision::float128, 10>(fs);
    write_grid<boost::multiprecision::float128, 11>(fs);
    write_grid<boost::multiprecision::float128, 12>(fs);
    write_grid<boost::multiprecision::float128, 13>(fs);
